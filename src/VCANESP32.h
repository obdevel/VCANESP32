...
https://github.com/SvenRosvall/VLCB-Arduino Licensed under the Creative Commons
Attribution-NonCommercial-ShareAlike 4.0 International License. The full licence
can be found at: http://creativecommons.org/licenses/by-nc-sa/4.0/

# Introduction to VCANESP32
This library provides a software CAN interface on an ESP32 device when used in
conjunction with the VLCB_Arduino library suite.  The main VLCB-Arduino library
can be found at [VLCB](https://github.com/SvenRosvall/VLCB-Arduino).

See [Design
documents](https://github.com/SvenRosvall/VLCB-Arduino/blob/main/docs/Design.md)
for how this library is structured.

## Examples
There are two versions of the example that, from the users perspective, are
functionally identical. The first uses a single core in the Pico and is
identified as VLCB_4in4out_Pico_s, where 's' stands for single core.  The other
core will be dormant in a low power state.

The second version makes use of both cores in the processer and is identifed as
VLCB_4in4out_Pico_d, where 'd' stands for dual core.  This is organised such
that the VLCB library runs in core 0 and the application runs in core 1.

The docs folder in the VCANESP32 repository provides notes on how to use this
library with the 4in4out examples. It also holds a circuit schematic suitable
for the examples.

## Hardware

Currently supports the arduino-esp32 Arduino core
[arduino-esp32](https://github.com/espressif/arduino-esp32) Full instructions on
how to do this in the associated
